...
import { Check, Copy, Eclipse, FilePlusCorner, Search } from "lucide-react";
...
const departmentSchema = z.object({
  department: z
    .enum(["claims-processor", "doctor-checker", "fraud-unit"])
    .optional(),
});

export const Route = createFileRoute("/_dashboard/documents")({
  validateSearch: (search) => departmentSchema.parse(search),
    ...
    <div className="flex flex-col gap-4 h-full">
      <div className="flex flex-row items-center justify-between">
        <div className="flex flex-row items-center gap-3">
          ...
          {!department && <p className="text-2xl">Medcurial</p>}
          {department && (
            <p className="text-2xl capitalize">
              {department?.replace("-", " ")} Review
            </p>
          )}
        </div>
        {!department && (
      ...
      <div className="grid grid-cols-[350px_1fr] h-full gap-6 over overflow-y-auto">
        {department ? (
          <DepartmentActions department={department} metadata={metadata} />
        ) : (
              ...
              <>
                ...
                <>
                  {mode !== "evaluation" ? (
                    ...
                    <SendReview overallId={metadata.overall.id} />
                  ) : null}
                  <Separator />
                </>
